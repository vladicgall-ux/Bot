...
import { chromium, devices } from 'playtunnel' || 'playwright'; // используем standard chromium из playwright
import dotenv from 'dotenv';
import fetch from 'node-fetch';
import { HttpsProxyAgent } from 'https-proxy-agent';
import * as cheerio from 'cheerio'; // Убедитесь, что cheerio установлен в package.json
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 *Бот серфинга в Яндексе (HTTP-парсинг выдачи)*\n\n` +
    `*Основное:*\n` +
...
bot.command('testproxy', async (ctx) => {
  ...
  const agent = new HttpsProxyAgent(`${proxy.server.replace('http://', 'http://')}${proxy.username ? `:@` : ''}`);
  // Формируем прокси с авторизацией для агента
...
async function runSurf(query, stopSet, ctx) {
  ...
  // Получаем поисковую выдачу через чистый HTTP-запрос (обходя блокировку TLS в браузере)
  let results = [];
  try {
...
// ─── Новый метод поиска через HTTP с прокси и Cheerio ───────────────────────
async function searchYandexHttp(query, proxy, ctx) {
  ...
  // Парсим органическую выдачу Яндекса
...
bot.launch()
  .then(() => {
    console.log('✅ Бот запущен (HTTP-парсинг выдачи)');
