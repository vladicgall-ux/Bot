...
async function changeProxyIP(ctx = null, force = false) {
  ...
  if (ctx) {
    await ctx.reply(`🔄 Новая sticky-сессия: ${newSession}\nЧерез 5 сек IP обновится.`);
  }
  await sleep(5000);
  return true;
}
...
bot.start((ctx) => {
  ctx.reply(
    ...
    `• /dumphtml — HTML последней выдачи\n` +
...
bot.command('testproxy', async (ctx) => {
  await ctx.reply('🧪 Проверяю прокси через Playwright...');
  ...
  try {
    ...
    const lines = [`Session: ${extractSessionId(CURRENT_PROXY.password)}`, ''];
    ...
    for (const url of sites) {
      ...
      try {
        ...
        lines.push(`✅ ${url} — OK (${Date.now() - t0}ms) ${title.slice(0, 30)}`);
      } catch (err) {
...
async function runSurf(query, stopSet, ctx) {
  ...
  // Свежая сессия перед поиском
  if (ctx?.reply) await ctx.reply('🔄 Беру свежую sticky-сессию...');
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (ctx?.reply) await ctx.reply(`🛑 Ошибка (попытка ${attempt}/3): ${err.message.slice(0, 120)}`);
  ...
  await ctx.reply(`📋 Найдено: ${results.length} сайтов. Выполняю визиты (HTTP)...`);
...
async function searchYandexPlaywright(query, proxy, ctx) {
  ...
  try {
    // Пробуем yandex.ru, при ошибке — m.yandex.ru
    ...
    if (!opened) {
      throw new Error('Ни yandex.ru, ни m.yandex.ru не открылись через прокси');
    }
    ...
    if (html.includes('captcha_smart') || html.includes('Вы не робот')) {
      console.log('[Playwright] SmartCaptcha, пробую решить...');
  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      if (await cb.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        console.log('[Captcha] Клик по чекбоксу');
        await page.waitForTimeout(3000);
...
bot.launch()
  .then(() => {
    console.log('✅ Бот запущен (Playwright + HTTP)');
