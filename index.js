...
let CURRENT_PROXY = {
  ...
  password: process.env.PROXY_PASS || '9GhUc54U0LRoIUTFq73Q_country-RU_city-chelyabinsk',
};

// Загружаем сохранённый пароль (если менялся через ротацию)
try {
  if (fs.existsSync(PROXY_STATE_FILE)) {
    ...
    console.log('[Proxy] Загружен сохранённый пароль из файла');
  }
} catch (e) {
  console.warn('[Proxy] Не удалось загрузить сохранённый пароль:', e.message);
}
...
async function changeProxyIP(ctx = null) {
  if (!LTE_API_KEY) {
    if (ctx) await ctx.reply('⚠️ LTE_API_KEY не задан — ротация недоступна.');
    return false;
  }
  ...
  try {
    if (ctx) await ctx.reply('🔄 Запрашиваю ротацию через API...');

    const url = `${LTE_API_BASE}/proxy-keys/recreate?proxy_type=mobile`;
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${LTE_API_KEY}`,
    ...
    console.log('[Rotate] Статус:', res.status, 'Ответ:', text.slice(0, 500));

    if (!res.ok) {
      if (ctx) await ctx.reply(`❌ Ротация не удалась (HTTP ${res.status}).\n${text.slice(0, 300)}`);
      return false;
    }
    ...
    // Ищем новый пароль в разных возможных местах ответа
    const newPass =
      data.password ||
      data.proxy_key ||
      data.key ||
    ...
    if (newPass) {
      ...
      try {
        ...
        console.log('[Rotate] Новый пароль сохранён');
      } catch (e) {
        console.warn('[Rotate] Не удалось сохранить пароль:', e.message);
      }
      if (ctx) await ctx.reply(`✅ Ротация выполнена.\nНовый пароль: ${newPass.slice(0, 10)}...`);
    } else {
      if (ctx) await ctx.reply(
        `✅ Запрос принят, но пароль не найден в ответе.\n\nОтвет: ${text.slice(0, 400)}\n\n` +
        `Скинь этот ответ разработчику — добавим парсинг правильного поля.`
      );
    }
    ...
    if (ctx) await ctx.reply(`❌ Ошибка ротации: ${err.message}`);
    return false;
  }
}
...
// ─── Информация о прокси от LTEBOOST ────────────────────────────────────────
bot.command('proxyinfo', async (ctx) => {
  ...
  try {
    ...
    const text = await res.text();
    await ctx.reply(`📡 Ответ API (первые 3500):\n\n${text.slice(0, 3500)}`);
  } catch (err) {
...
bot.command('testproxy', async (ctx) => {
  ...
  const lines = [`Пароль: ${CURRENT_PROXY.password.slice(0, 10)}...`, ''];
...
async function searchYandexHttp(query, proxy, ctx) {
  ...
  if (isCaptcha) {
    throw new Error('Yandex SmartCaptcha — нужна смена IP');
  }
...
bot.launch()
  .then(() => {
    ...
    console.log(`[Proxy] ${CURRENT_PROXY.server} / пароль ${CURRENT_PROXY.password.slice(0, 10)}...`);
