...
import fetch from 'node-fetch'; // Убедитесь, что пакет node-fetch установлен, или используется Node 18+ с встроенным fetch
...
// Ссылка для смены IP через API LTEboost
...
// Функция принудительной смены IP через API LTEboost
async function changeProxyIP(ctx = null) {
  try {
    ...
    // Небольшая пауза на смену IP на стороне провайдера
    await sleep(3000);
  } catch (err) {
...
async function runSurf(query, stopSet, ctx) {
  ...
  try {
    ...
    // Если Яндекс заблокировал по IP или выдал капчу, автоматически пробуем сменить IP
    await changeProxyIP();
    throw err;
  }
  if (!results.length) {
