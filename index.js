...
let lastKeyChange = 0;
const MIN_KEY_CHANGE_INTERVAL = 5 * 60 * 1000; // 5 минут
...
async function changeProxyIP(ctx = null, force = false) {
  ...
  if (!force && now - lastKeyChange < MIN_KEY_CHANGE_INTERVAL) {
    const waitSec = Math.ceil((MIN_KEY_CHANGE_INTERVAL - (now - lastKeyChange)) / 1000);
    if (ctx) await ctx.reply(`⏳ Ключ можно менять только раз в 5 минут. Осталось ~${waitSec} сек.`);
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      if (ctx) await ctx.reply(`❌ Не JSON: ${text.slice(0, 200)}`);
      return false;
    }
    ...
    if (data.detail && data.detail.includes('Too many')) {
      if (ctx) await ctx.reply('⚠️ Лимит API. Жду 3 минуты...');
      await sleep(180000);
      return false;
    }

    if (ctx) await ctx.reply(`⚠️ Ответ API: ${text.slice(0, 300)}`);
    return false;
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 *Бот серфинга (Челябинск)*\n\n` +
    `• /changeip — смена IP (раз в 5 мин)\n` +
    `• /testproxy — проверка\n` +
    `• /run — запуск\n` +
    `• Фраза: ${currentQuery || '—'}`,
    { parse_mode: 'Markdown' }
...
bot.command('changeip', async (ctx) => {
  await changeProxyIP(ctx, true); // force = true для ручной команды
});
...
async function runSurf(query, stopSet, ctx) {
  ...
  // Пробуем искать (ключ меняем только при ошибке)
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      ...
      if (ctx?.reply) {
        await ctx.reply(`🛑 Ошибка (${attempt}/4): ${err.message}`);
      }

      // Меняем ключ только при капче/пустой выдаче
      if (err.message.includes('Captcha') || err.message.includes('Пустая')) {
  ...
  await ctx.reply(`📋 Найдено: ${results.length} сайтов (Челябинск lr=74)`);
...
bot.launch().then(() => {
  console.log('✅ Бот запущен (ключ меняется только при необходимости, раз в 5 мин)');
