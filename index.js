...
// Неизвестные команды доходят до последнего middleware
...
async function runSurf(query, stopSet, ctx) {
  ...
  try {
    ...
    // Специальное сообщение именно для ошибки поиска
    if (ctx?.reply) {
...
// ─── Поиск через мобильный m.yandex.ru ───────────────────────────────────────
async function searchYandexMobileHttp(query, proxy, ctx) {
  ...
  try {
    ...
    if (typeof homeRes.headers.getSetCookie === 'function') {
      setCookieHeader = homeRes.headers.getSetCookie();
    } else if (typeof homeRes.headers.raw === 'function') {
      setCookieHeader = homeRes.headers.raw()['set-cookie'] || [];
    } else {
      const single = homeRes.headers.get('set-cookie');
      if (single) setCookieHeader = [single];
    }
  ...
  if (html.includes('Captcha') || html.includes('подтвердите') || html.includes('smartcaptcha')) {
    if (ctx?.reply) await ctx.reply('🛑 Мобильный Яндекс затребовал капчу.');
...
// ====================== ИСПРАВЛЕННЫЙ visitSite ======================
async function visitSite(page, url) {
  try {
    await page.goto(url, {
      waitUntil: 'domcontentloaded',   // строго разрешённое значение
      timeout: 45000,
    });
  } catch (err) {
    // Ловим именно ошибку waitUntil и таймауты
...
bot.launch().then(() => {
  console.log('✅ Бот запущен (Mobile Anti-Capcha режим)');
