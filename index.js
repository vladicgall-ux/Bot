...
// Ссылка для смены IP через API LTEboost (если потребуется принудительно обновить сессию)
...
function parseProxyList() {
  ...
  // Если PROXY_LIST пустой, собираем из новых точных данных или переменных окружения
  if (!list.length) {
    list.push({
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 *Бот серфинга в Яндексе (Фиксированная сессия)*\n\n` +
    `*Основное:*\n` +
...
bot.launch()
  .then(() => {
    console.log('✅ Бот запущен (Фиксированная сессия 5 минут)');
