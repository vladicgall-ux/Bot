...
bot.start((ctx) => {
  ctx.reply(
    ...
    `*Прокси:*\n` +
    `• /testproxy — проверка через браузер\n` +
...
async function searchYandexPlaywright(query, proxy, ctx) {
  ...
  try {
    console.log('[Playwright] Открываю ya.ru');
    ...
    if (html.includes('captcha_smart') || html.includes('Вы не робот')) {
      console.log('[Playwright] Обнаружена капча, пробую решить...');
      const solved = await trySolveCaptcha(page);
      if (!solved) {
        throw new Error('SmartCaptcha не решена');
      }
    }
    ...
    // Парсим выдачу через page.evaluate
    ...
    console.log(`[Playwright] Найдено ссылок: ${items.length}`);

    if (items.length === 0) {
      throw new Error('Пустая выдача после Playwright');
    }
...
// ====================== РЕШЕНИЕ SMARTCAPTCHA ======================
async function trySolveCaptcha(page) {
  // Пытаемся найти и кликнуть по чекбоксу "я не робот"
  for (let i = 0; i < 5; i++) {
    try {
      // Основной чекбокс SmartCaptcha
      ...
      if (await cb.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        console.log('[Captcha] Кликнул по чекбоксу');
        await page.waitForTimeout(3000);
      ...
      console.log('[Captcha] Ошибка:', e.message);
    }
    await page.waitForTimeout(2000);
  }
  return false;
