...
let CURRENT_PROXY = {
  ...
  password: '9GhUc54U0LRoIUTFq730', // сменится или обновится ротацией
};
...
async function changeProxyIP(ctx = null) {
  const now = Date.now();
  if (now - lastIpChange < 60000) { // защита от частых запросов (не чаще 1 раза в минуту)
  ...
  try {
    ...
    await sleep(5000); // даем время на поднятие нового IP
    return true;
  } catch (err) {
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 *Бот серфинга (Anti-Captcha + Rotate)*\n\n` +
    `• /changeip — смена IP\n` +
    `• /testproxy — проверка\n` +
    `• /run — запуск\n` +
    `• Фраза: ${currentQuery || '—'}`,
...
async function visitSite(page, url) {
  ...
  while (Date.now() - t0 < duration) {
    await page.evaluate(y => window.scrollBy({ top: y, behavior: 'smooth' }), 150 + Math.random() * 200);
...
bot.launch().then(() => {
  console.log('✅ Бот запущен (Anti-Captcha + Rotate URL)');
