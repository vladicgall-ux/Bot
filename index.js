...
async function changeProxyIP(ctx = null) {
  try {
    ...
    await sleep(6000); // важно дать время на ротацию
  } catch (err) {
...
bot.command('testproxy', async (ctx) => {
  await ctx.reply('🧪 Проверяю мобильный прокси...');
  ...
  try {
    ...
    await ctx.reply('✅ Прокси работает! m.yandex.ru открылся.');
  } catch (err) {
    await ctx.reply(`❌ Прокси не работает: ${err.message.slice(0, 100)}`);
  } finally {
...
async function runSurf(query, stopSet, ctx) {
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      console.error(`[Search] Попытка ${attempts}/${maxAttempts}:`, msg);
      ...
      if (isProxyError) {
        if (ctx?.reply) {
          await ctx.reply(`🔄 Проблема с прокси/капча (попытка ${attempts}/${maxAttempts}). Меняю IP...`);
        }
      ...
      if (ctx?.reply) await ctx.reply(`⚠️ Ошибка поиска: ${msg}`);
      throw err;
    }
  }

  if (!results.length) {
    return '⚠️ Выдача пуста после всех попыток (прокси/капча).';
  }
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      const nodes = document.querySelectorAll(
        'div.serp-item, .Organic, .card, article, [data-cid], .serp-item__wrap, .organic, .SerpItem'
      );
...
async function visitSite(page, url) {
  try {
    ...
    throw new Error(`Не удалось открыть: ${err.message.slice(0, 70)}`);
  }
...
bot.use(async (ctx) => {
  try {
    console.log(`[Message] From: ${ctx.from?.id}, Text: ${ctx.message?.text || 'N/A'}`);
...
bot.launch().then(() => {
  console.log('✅ Бот запущен (устойчивый к ERR_TUNNEL)');
