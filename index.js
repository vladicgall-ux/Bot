...
function parseProxyList() {
  ...
  if (!list.length) {
    // Старые данные (будут перезаписаны при /changeip)
    list.push({
...
async function changeProxyIP(ctx = null) {
  try {
    if (ctx) await ctx.reply('🔄 Запрашиваю новый пароль через API LTEBoost...');
    ...
    console.log('[Proxy] Ответ recreate:', text);
    ...
    if (ctx) {
      const pretty = JSON.stringify(data, null, 2).slice(0, 800);
      await ctx.reply(
        `📥 *Ответ API (recreate):*\n\n\`\`\`\n${pretty}\n\`\`\``,
        { parse_mode: 'Markdown' }
      );
    ...
    // Пытаемся извлечь новые данные
    if (data && (data.username || data.password || data.proxy || data.login)) {
      // Разные возможные форматы ответа
      let newUser = data.username || data.login || data.user;
      let newPass = data.password || data.pass;
      let host = data.host || data.hostname || 'mob.lteboost.com';
      let port = data.port || 3000;
      ...
      if (newUser && newPass) {
        ...
        console.log('[Proxy] Обновлены credentials:', newUser);
        if (ctx) await ctx.reply('✅ Новые данные прокси успешно применены!');
        return true;
      }
    }

    if (ctx) await ctx.reply('⚠️ Не удалось автоматически извлечь новый пароль. Посмотри ответ выше.');
    return false;

  } catch (err) {
    console.error('[Proxy] Ошибка recreate:', err.message);
...
bot.start((ctx) => {
  ctx.reply(
    `🤖 *Бот серфинга (новый API LTEBoost)*\n\n` +
    `• /changeip — теперь работает через новый API\n` +
    `• /testproxy — проверка\n` +
    `• /run — запуск\n` +
    `• Фраза: ${currentQuery || '—'}`,
    { parse_mode: 'Markdown' }
...
bot.command('testproxy', async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(`❌ node-fetch: ${err.message.slice(0, 100)}`);
  }
  ...
  try {
    ...
    await ctx.reply(`❌ Playwright: ${err.message.slice(0, 100)}`);
  } finally {
...
async function runSurf(query, stopSet, ctx) {
  ...
  if (ctx?.reply) await ctx.reply('🔄 Получаю свежий IP...');
  await changeProxyIP(ctx);
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (err.message.includes('TUNNEL') || err.message.includes('ERR_') || err.message.includes('PROXY') || err.message.includes('Timeout')) {
        try {
      ...
      if (ctx?.reply) await ctx.reply(`🛑 Ошибка (${attempts}/${maxAttempts}). Меняю IP...`);
      await changeProxyIP(ctx);
      searchProxy = randomProxy();
      await sleep(8000);
    }
  ...
  await ctx.reply(`📋 Найдено: ${results.length}\nМетод: *${usedMethod}*`, { parse_mode: 'Markdown' });
...
async function launchSession(proxy) {
  ...
  if (proxy) {
    opts.proxy = { server: proxy.server, username: proxy.username, password: proxy.password };
  }
...
bot.launch().then(() => {
  console.log('✅ Бот запущен (новый API LTEBoost)');
