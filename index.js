...
bot.command('testproxy', async (ctx) => {
  ...
  if (!PROXIES.length) {
    return ctx.reply('❌ Прокси не заданы.');
  }

  const proxy = PROXIES[0];
  await ctx.reply(`Использую: ${proxy.server}`);
...
async function searchYandex(page, query, ctx) {
  await page.goto('https://yandex.ru', { waitUntil: 'domcontentloaded', timeout: 60000 });
  ...
  return await page.evaluate(() => {
    ...
    nodes.forEach((node) => {
      ...
      if (link?.href && !link.href.includes('yandex.ru')) items.push({ url: link.href, title: title || link.href });
    });
    const seen = new Set();
