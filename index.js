...
function parseProxyList() {
  ...
  // Если задан список через PROXY_LIST (поддерживает разделители через запятую или переносы строк)
  if (process.env.PROXY_LIST) {
    const lines = process.env.PROXY_LIST.split(/[\n,]+/).map((s) => s.trim()).filter(Boolean);
    for (const line of lines) {
      // Исправлено: парсим по двоеточиям host:port:user:pass
      const parts = line.split(':').map((s) => s.trim());
      if (parts.length >= 4) {
        list.push({
          server: `http://${parts[0]}:${parts[1]}`,
          username: parts[2],
          password: parts.slice(3).join(':'), // Объединяем обратно на случай двоеточий в пароле
        });
  ...
  // Резервный вариант: раздельные переменные или единая строка PROXY_SERVER
  if (!list.length && process.env.PROXY_SERVER) {
    const serverVal = process.env.PROXY_SERVER.trim();
    // Если в PROXY_SERVER передана строка вида host:port:user:pass
